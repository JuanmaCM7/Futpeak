...
def main():
    ...
    def _load_custom_css():
        ...
        st.markdown("""<style> .stImage > div { border: none !important; box-shadow: none !important; } </style>""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.info("""1. Selecciona un jugador del menú desplegable.  
...
3. Compara con grupos de jugadores similares.""")
        ...
        st.markdown("<p style='margin:0 0 0.4rem 0; white-space: nowrap; color:#ffffff; font-size:1rem;'>👤 Selecciona un jugador:</p>", unsafe_allow_html=True)
        selected_player = st.selectbox("👤 Selecciona un jugador:", options=player_names, index=0, label_visibility="collapsed")

        st.markdown("""
        <style>div[data-baseweb="menu"] [role="option"] { color: black !important; }</style>
        """, unsafe_allow_html=True)
...
# ✅ Solo esto va fuera de la función
